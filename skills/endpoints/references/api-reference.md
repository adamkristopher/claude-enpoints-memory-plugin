...
### POST /api/endpoints/{category}/{slug}

Upsert endpoint - creates if it doesn't exist, appends items if it does. Ideal for memory operations.

**Request:**
```json
{
  "items": [
    { "topic": "My Research", "key_findings": ["Finding 1", "Finding 2"] }
  ]
}
```
...
Returns `201 Created` when creating a new endpoint, `200 OK` when appending.
